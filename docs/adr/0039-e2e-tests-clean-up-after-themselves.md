...
| Event | 1 | Name `e2e-retained-event` - the literal constant, distinguishable from every transient `e2e-<label>-<guid>` Event by carrying no guid suffix. Status `Draft` (P2-20, #115) - created name-only through the API and never marked Live, so this is already true and needs no seeding change |
...
| Deleted User (P2-19, #114) | 0 | An activated Director a delete scenario creates on its own (`CreateTrackedUserAsync`, never the retained `e2e-director` fixture account) and then deletes through the UI action under test - self-cleaning by the assertion path itself, same as the deleted-Event and removed-Grant rows above |
| Live/Cancelled Event (P2-20, #115) | 0 | An Event a status-change scenario creates via `CreateEventAsync` and takes Live/Cancelled through the UI action under test - unlike the deleted-Event row above, cancelling doesn't self-clean (the row survives, just with a different Status), so this relies on the standard `TrackEvent`+`DisposeAsync` teardown every throwaway Event already gets, not a special case |
| Duplicated Event (P2-21, #116) | 0 | An Event a duplicate scenario creates by clicking Duplicate on a source Event's row and completing the reused "New event" form (ADR-0054 - Duplicate never auto-generates a Name, so the scenario types an ordinary `e2e-`-prefixed one itself, already matched by the existing sweep filter with no widening needed). Relies on the standard `TrackEvent`+`DisposeAsync` teardown, same as the Live/Cancelled Event row above - nothing about landing on this page through Duplicate instead of "+ New event" self-cleans it |
