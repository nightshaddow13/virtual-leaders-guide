...
need individual identities. We decided on a single shared Passcode per Event, checked against a per-event hash,
granting access via a signed cookie scoped to that event (no per-visitor account, sign-up, or profile).
