...
/// <see cref="ChangePasswordScenarios"/> instead. The delete fact seeds a plain no-role user - a Director's
/// or Admin's Role grant cascade-deleting along with them is DB-enforced FK behavior
/// (<c>UserRole.UserId</c>'s <c>OnDelete(DeleteBehavior.Cascade)</c>), not application logic, so it isn't
/// this suite's job to prove.
/// </remarks>
