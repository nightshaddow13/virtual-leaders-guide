...
public class PageEntitiesAreNotJsonApiResourcesShould : NonResourceEntityShouldBase
{
    ...
    /// <remarks>
    /// The 404 probe above can't distinguish "removed from the resource graph" from "never had a controller" -
    /// both look identical over HTTP. This pins the graph-membership half of ADR-0059's decision directly, so
    /// a future change that stops calling <c>Remove&lt;Page&gt;()</c> fails here even though <c>/api/pages</c>
    /// would still 404 (no <c>[Resource]</c> on <c>Page</c> either way).
    /// </remarks>
    [Fact]
